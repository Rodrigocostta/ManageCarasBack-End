...
import com.manageCars.manageCars.DTOs.OrdemServicoDTO;
...
public class OrdemServicoResource {
	...
	@GetMapping
	ResponseEntity<List<OrdemServicoDTO>> findAll() {

		List<OrdemServicoDTO> list = service.findAll().stream().map(p -> new OrdemServicoDTO(
			    
				p.getCliente().getNome(),
				p.getVeiculo().getModelo(),
				p.getVeiculo().getPlaca(),
				p.getServico().getDescricao(),
				p.getSubTotal())).toList();
	...
	@DeleteMapping(value = "/{cont}")
	public ResponseEntity<Void> delete(@PathVariable Long cont) {
		service.delete(cont);
